...
function submitRideRequest() {
  ...
  // 1) حفظ في لوحة المدير
  const requests = JSON.parse(localStorage.getItem('lahza_requests') || '[]');
  requests.push({ id: Date.now(), name, phone, from, to, type, notes, status: 'pending', ts: Date.now() });
  localStorage.setItem('lahza_requests', JSON.stringify(requests));
